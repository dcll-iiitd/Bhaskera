...
class Param2TurboQuantCache:
    """
    Wraps HF DynamicCache with TurboQuant compression.
    
    param2moe's custom forward calls cache.update() with the standard
    (key, value, layer_idx) signature — same as DynamicCache.
    We intercept those calls, compress with TurboQuant, store compressed,
    and return decompressed tensors so attention sees full-precision KV.
    """

    def __init__(self, tq_cache):
        self._tq = tq_cache
        self._seq_lens = {}
    ...
    def update(
        ...
        full_k, full_v = self._tq.update(key_states, value_states, layer_idx, cache_kwargs)
