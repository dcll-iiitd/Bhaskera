...
def _get_default_orthogonalize_fn() -> Callable[[Tensor], Tensor]:
    ...
    if _DEFAULT_ORTHOGONALIZE_FN is None:
        base_fn = _make_default_orthogonalize_fn()
        def wrapped_fn(x: Tensor) -> Tensor:
            if min(x.shape[-2:]) <= 128:
                import math
                return (x / x.norm().clamp(min=1e-8)) * math.sqrt(x.numel())
            return base_fn(x)
        _DEFAULT_ORTHOGONALIZE_FN = wrapped_fn
    return _DEFAULT_ORTHOGONALIZE_FN
